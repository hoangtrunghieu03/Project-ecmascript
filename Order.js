...
for (const polyshop of cartItems) {
  list += `
  <div class="d-flex align-items-center pb-2 border-bottom">
    <a class="d-block flex-shrink-0 mr-2" href="#"
      ><img
        class="rounded-1"
        src="${polyshop.product.productImg}"
        width="64"
        alt="${polyshop.product.productName}"
    /></a>
    <div class="ps-1">
      <h6 class="widget-product-title">
        <a href="#">${polyshop.product.productName}</a>
      </h6>
      <div
        class="d-flex justify-content-between align-items-center"
      >
        <span class="text-accent border-end pr-2 mr-2"
          >${formatNumber(polyshop.product.productPrice * polyshop.quantity)}</span
        ><span class="text-end">${polyshop.quantity}</span>
      </div>
    </div>
  </div>
`;
